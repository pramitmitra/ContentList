#!/bin/ksh -eu
#------------------------------------------------------------------------------------------------
# Filename:     single_hadoop_extract_file.ksh
#
# Revision History:
#
...
HADOOP_EXTRACT_JOB="$DW_EXE/hadoop_extract_file.ksh"
...
do
  ...
  then
    ...
    then
      ...
      print "(/usr/bin/ssh -q $HOST_NAME \"ksh $HADOOP_EXTRACT_JOB $ETL_ID $JOB_ENV $UOW_ID $HDP_CONN $RECORD_ID $instance_idx $SRC_HOST_CNT $CURR_DATETIME\" > $LOG_FILE 2>&1) || (print \"ERROR - Failure run hadoop_extract_file process on $HOST_NAME\" >>$LOG_FILE) &" >>$COMMAND_SCRIPT
    else
      print "print \"Launching instance $instance_idx on $HOST_NAME ...\"" >> $COMMAND_SCRIPT
      print "ksh $HADOOP_EXTRACT_JOB $ETL_ID $JOB_ENV $UOW_ID $HDP_CONN $RECORD_ID $instance_idx $SRC_HOST_CNT $CURR_DATETIME > $LOG_FILE 2>&1 &" >> $COMMAND_SCRIPT
    fi
  fi
