...
###################################################################################################################
#
# Title:        Target Table Transform Handler 
# File Name:    target_table_transform_handler.ksh 
# Description:  Handler for Spark Transform Job
# Developer:    Pramit Mitra 
# Created on:   Legacy code re-purposed from traget_table_load_handler.ksh 
# Location:     $DW_EXE
#
...
## Added JOB_ENV check for Spark environment 
if [[ $JOB_ENV == sp* ]]
then
  print "Inside Spark Env Check Module"
  export JOB_TYPE=spark
  export JOB_TYPE_ID=sp
elif [[ $JOB_ENV == hd* ]]
then
...
#Commenting out this extra call as its interfering Spark Submit
#. /dw/etl/mstr_cfg/etlenv.setup
...
#  This module call newly build target_table_load_run_spark.ksh script for Spark
#
######################################################################################################
...
then
   ...
   (time $DW_MASTER_BIN/target_table_load_run_spark.ksh) > $PARENT_LOG_FILE 2>&1
#time $DW_MASTER_BIN/target_table_load_run_spark.ksh
   RUN_RCODE=$?
