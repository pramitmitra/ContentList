...
set +e
ssh "$SCP_USERNAME@$SCP_HOST" "cd $REMOTE_DIR; /usr/local/bin/wget --verbose --append-output=$ETL_ID.$SOURCE_FILE_LOG.$CURR_DATETIME.log --tries=5 --waitretry=2 --no-host-directories $URL/$SOURCE_FILE_TMP" >&2
rcode=$?
set -e
...
then
        ...
        print
        print \"$SCP_USERNAME@$SCP_HOST \"cd $REMOTE_DIR\; /usr/local/bin/wget --verbose --append-output=$ETL_ID.$SOURCE_FILE_LOG.$CURR_DATETIME.log --tries=5 --waitretry=2 --no-host-directories $URL/$SOURCE_FILE_TMP\" >&2
        print
