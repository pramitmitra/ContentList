#!/bin/ksh -eu
#------------------------------------------------------------------------------------------------
# Title:        Hadoop load file Handler
# File Name:    hadoop_load_file_handler.ksh
# Description:  Handler submits multiple instances of load job on different hosts.
# Developer:
# Created on:
# Location:     $DW_BIN
# Logic:
...
while getopts "i:e:u:" opt
do
  case $opt in
    i)   _etl_id="$OPTARG";;
    e)   _job_env="$OPTARG";;
    u)   _uow_id="$OPTARG";;
    \?)  print >&2 "Usage: $0 -i <ETL_ID> -e <JOB_ENV> -u [UOW_ID]"
    return 1;;
  esac
done
shift $(($OPTIND - 1))
...
if [[ X"$_etl_id" = X"" || X"$_job_env" = X"" ]]
then
  print "Usage: $0 -i <ETL_ID> -e <JOB_ENV> -u [UOW_ID]"
  exit 4
fi
export ETL_ID=$_etl_id
export JOB_ENV=$_job_env
export UOW_ID=${_uow_id:-""}

export JOB_TYPE="hdp_file_load"
...
TABLE_LIS_FILE=$DW_CFG/$ETL_ID.targets.lis
PARENT_ERROR_FILE=$DW_SA_LOG/$TABLE_ID.$JOB_TYPE_ID.hadoop_load_file_handler.$CURR_DATETIME.err
...
##########################################################################################################
#
# Beginning Hadoop file load for ETL_ID: $ETL_ID, UOW_ID: $UOW_ID   `date`
#
###########################################################################################################
"
...
then
  ...
  do
    ...
    print "(/usr/bin/ssh -q $HOST_NAME \"$DW_EXE/loader_cleanup_sg.ksh $JOB_ENV $JOB_TYPE_ID $ETL_ID $CURR_DATETIME\" 2>$LOG_FILE) || (print \"ERROR - Failure run loader_cleanup process on $HOST_NAME\" >>$LOG_FILE) &">>$COMMAND_SCRIPT
    host_idx=$(( $host_idx + 1 ))
  done
...
then
  ...
  if [[ $TABLE_COUNT -eq 1 ]]
  then
    print "Processing single hadoop file load for TABLE_ID: $TABLE_ID  `date`"

    read RECORD_ID HDP_CONN PARALLEL_NUM SOURCE_FILE TARGET_FILE MISC < $TABLE_LIS_FILE

    LOG_FILE=$DW_SA_LOG/$TABLE_ID.$JOB_TYPE_ID.single_hadoop_load_file.$CURR_DATETIME.log
    set +e
    eval $DW_EXE/single_hadoop_load_file.ksh $ETL_ID $RECORD_ID $HDP_CONN > $LOG_FILE 2>&1
    #print "$DW_EXE/single_hadoop_load_file.ksh $ETL_ID $RECORD_ID $HDP_CONN '$SOURCE_FILE' '$TARGET_FILE' > $LOG_FILE 2>&1"
    rcode=$?
    set -e
  ...
  then
    print "Processing multiple hadoop file load for TABLE_ID: $TABLE_ID `date`"

    export MULTI_COMP_FILE=$DW_SA_TMP/$TABLE_ID.multi_hadoop_load_file.complete
    export PARENT_LOG_FILE=$DW_SA_LOG/$TABLE_ID.$JOB_TYPE_ID.run_hadoop_load_file.$CURR_DATETIME.log
    export ERROR_FILE=$DW_SA_LOG/$TABLE_ID.$JOB_TYPE_ID.multi_hadoop_load_file.$CURR_DATETIME.err  # job error file
    ...
    do
      ...
      LOG_FILE=$DW_SA_LOG/$TABLE_ID.$JOB_TYPE_ID.$HDP_CONN.run_multi_hadoop_load_file.$CURR_DATETIME.log
      print "Running run_multi_hadoop_load_file.ksh $FILE  `date`"
      COMMAND="$DW_EXE/run_multi_hadoop_load_file.ksh $FILE $PARENT_LOG_FILE > $LOG_FILE 2>&1"
