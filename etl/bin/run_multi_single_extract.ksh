#!/bin/ksh -eu
#############################################################################################################
# Title:        Run Multi Single Extract
# File Name:    run_multi_single_extract.ksh
# Description:  Run multiple extract in throttled parallel
# Developer:    ???
# Created on:
# Location:     $DW_EXE
# Logic:
...
do
	...
	then
		...
		do
			...
			then
				if [ -f $DW_SA_TMP/$ETL_ID.ex.$CONN_FILE.restart ]
				then 
		...
		COMMAND="$DW_EXE/single_${EXTRACT_TYPE}_extract.ksh $ETL_ID $FILE_ID $CONN_FILE $SOURCE_NAME $DATA_FILENAME $UOW_PARAM_LIST_AB $PARAM_LIST > $LOG_FILE 2>&1"
...
then
	...
	do
		...
		then
			...
			print "due to existence of $DW_SA_TMP/$ETL_ID.ex.$CONN_FILE.restart restart file" 
