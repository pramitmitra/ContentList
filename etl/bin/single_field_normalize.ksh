...
# Deployed execution script for graph "single_field_normalize", compiled at Friday, October 04, 2013 11:09:03 using GDE version 3.0.3.1
export AB_JOB;AB_JOB=${AB_JOB_PREFIX:-""}single_field_normalize
...
export AB_GRAPH_NAME;AB_GRAPH_NAME=single_field_normalize

# Host Setup Commands:
. /dw/etl/mstr_cfg/etlenv.setup
_AB_PROXY_DIR=single_field_normalize-ProxyDir-$$
rm -rf "${_AB_PROXY_DIR}"
mkdir "${_AB_PROXY_DIR}"
...
print -r -- 'Usage: single_field_normalize.ksh <TRGT_ETL_ID> <SRC_BATCH_SEQ_NUM> <SRC_ETL_ID> <SRC_JOB_ENV> <SRC_JOB_TYPE> <SRC_JOB_TYPE_ID> <INPUT_DML_FILENAME> <OUTPUT_DML_FILENAME>'
exit 1
fi
...
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter TRGT_SUBJECT_AREA of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
export TRGT_TABLE_ID;TRGT_TABLE_ID=${TRGT_ETL_ID##*.}
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter TRGT_TABLE_ID of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
...
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter SRC_SUBJECT_AREA of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
export SRC_TABLE_ID;SRC_TABLE_ID=${SRC_ETL_ID##*.}
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter SRC_TABLE_ID of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
export CURR_DATETIME;CURR_DATETIME=${CURR_DATETIME:-$(date '+%Y%m%d-%H%M%S')}
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter CURR_DATETIME of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
export CURR_DATE;CURR_DATE=${CURR_DATE:-${CURR_DATETIME%-*}}
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter CURR_DATE of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
export IS_CATY;IS_CATY=$(grep "^IS_CATY\>" $SRC_ETL_CFG_FILE | read PARAM VALUE COMMENT; print $VALUE)
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter IS_CATY of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
export IN_DIR;IN_DIR=$(
grep "^IN_DIR\>" $SRC_ETL_CFG_FILE | read PARAM VALUE COMMENT; 
if [[ "X$UOW_TO" != "X" ]]
then
   eval print $VALUE/$SRC_JOB_ENV/$SRC_SUBJECT_AREA/$SRC_TABLE_ID/$UOW_TO_DATE/$UOW_TO_HH/$UOW_TO_MI/$UOW_TO_SS
else
   eval print $VALUE/$SRC_JOB_ENV/$SRC_SUBJECT_AREA
fi
)
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter IN_DIR of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
export TRGT_IN_DIR;TRGT_IN_DIR=$(
grep "^IN_DIR\>" $TRGT_ETL_CFG_FILE | read PARAM VALUE COMMENT; 
if [[ "X$UOW_TO" != "X" ]]
then
   eval print $VALUE/$SRC_JOB_ENV/$TRGT_SUBJECT_AREA/$TRGT_TABLE_ID/$UOW_TO_DATE/$UOW_TO_HH/$UOW_TO_MI/$UOW_TO_SS
   if [[ ! -d $VALUE/$SRC_JOB_ENV/$TRGT_SUBJECT_AREA/$TRGT_TABLE_ID/$UOW_TO_DATE/$UOW_TO_HH/$UOW_TO_MI/$UOW_TO_SS ]]
   then
      mkdir -p $VALUE/$SRC_JOB_ENV/$TRGT_SUBJECT_AREA/$TRGT_TABLE_ID/$UOW_TO_DATE/$UOW_TO_HH/$UOW_TO_MI/$UOW_TO_SS
   fi
   
else
   eval print $VALUE/$SRC_JOB_ENV/$TRGT_SUBJECT_AREA
fi
)
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter TRGT_IN_DIR of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
...
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter CNDTL_EXTRACT_COMPRESS_LEVEL of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
...
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter CNDTL_EXTRACT_COMPRESS of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
...
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter CNDTL_EXTRACT_NOT_COMPRESS of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
...
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter CNDTL_SRC_EXTRACT_COMPRESS of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
export EXTRACT_PROCESS_TYPE;EXTRACT_PROCESS_TYPE=$(grep "^EXTRACT_PROCESS_TYPE\>" $SRC_ETL_CFG_FILE | read PARAM VALUE COMMENT; print $VALUE)
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter EXTRACT_PROCESS_TYPE of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
export INPUT_FILE;INPUT_FILE=$( INPUT_FILE_TMP=""
   ...
   then
      if [[ -n $UOW_TO ]]
      then
         SRC_DATA_LIS=$DW_SA_DAT/$ETL_ID.sources.lis.$UOW_TO
      else
         SRC_DATA_LIS=$DW_SA_DAT/$ETL_ID.sources.lis.$BATCH_SEQ_NUM
      fi
      ...
      do
         if [[ -n $UOW_TO ]]
         then
            DATA_FILENAME_TMP=$DATA_FILENAME$COMP_SFX
         else
            DATA_FILENAME_TMP=$DATA_FILENAME.$SRC_BATCH_SEQ_NUM$COMP_SFX
         fi

         INPUT_FILE_TMP="$INPUT_FILE_TMP $(eval print $IN_DIR/$DATA_FILENAME_TMP)"
      done < $SRC_DATA_LIS
   elif [ $IS_CATY != 1 ]
   then   
      ...
      do
         if [[ -n $UOW_TO ]]
         then
            DATA_FILENAME_TMP=$DATA_FILENAME
         else
            DATA_FILENAME_TMP=$DATA_FILENAME.$SRC_BATCH_SEQ_NUM
         fi

         if [[ CNDTL_SRC_EXTRACT_COMPRESS = 1  && -f $(eval print $IN_DIR/$DATA_FILENAME_TMP.Z) ]]
         then
            COMP_SFX=.Z
         fi

         if [ -f $(eval print ${IN_DIR}/.${DATA_FILENAME_TMP}${COMP_SFX}.mfctl) ]
         then
            INPUT_FILE_TMP="$INPUT_FILE_TMP $(eval print $(m_expand $IN_DIR/$DATA_FILENAME_TMP$COMP_SFX))"
         else
            INPUT_FILE_TMP="$INPUT_FILE_TMP $(eval print $IN_DIR/$DATA_FILENAME_TMP$COMP_SFX)"
         fi
      ...
      do
         ...
         if [[ CNDTL_SRC_EXTRACT_COMPRESS = 1  && -f $(eval print $IN_DIR/$DATA_FILENAME.Z) ]]
         then
            COMP_SFX=.Z
         fi

         if [ -f $(eval print ${IN_DIR}/.${DATA_FILENAMETMP}${COMP_SFX}.mfctl) ]
         then
            INPUT_FILE_TMP="$INPUT_FILE_TMP $(eval print $(m_expand $IN_DIR/$DATA_FILENAME_TMP$COMP_SFX))"
         else
            INPUT_FILE_TMP="$INPUT_FILE_TMP $(eval print $IN_DIR/$DATA_FILENAME_TMP$COMP_SFX)"
...
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter INPUT_FILE of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
...
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter OUTPUT_FILE of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
export INPUT_DML;INPUT_DML="$DW_DML"'/'"$INPUT_DML_FILENAME"
export OUTPUT_DML;OUTPUT_DML="$DW_DML"'/'"$OUTPUT_DML_FILENAME"
export REFORMAT_DML_FILE;REFORMAT_DML_FILE="$DW_DML"'/'"$TRGT_ETL_ID"'.reformat.write.dml'
export REFORMAT_XFR_FILE;REFORMAT_XFR_FILE=$( if [ -f $DW_XFR/$TRGT_ETL_ID.normalize.reformat.xfr ]
   then
      print $DW_XFR/$TRGT_ETL_ID.normalize.reformat.xfr
   else
      print $DW_XFR/$TRGT_ETL_ID.reformat.xfr
   fi
)
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter REFORMAT_XFR_FILE of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
export REDEFINE_FORMAT_DML_FILE;REDEFINE_FORMAT_DML_FILE="$DW_DML"'/'"$TRGT_ETL_ID"'.redefine_format.write.dml'
export NORMALIZE_XFR_FILE;NORMALIZE_XFR_FILE="$DW_XFR"'/'"$TRGT_ETL_ID"'.normalize.xfr'
export LOG_FILE_DIR;LOG_FILE_DIR=$DW_LOG/$SRC_JOB_ENV/$TRGT_SUBJECT_AREA/$TRGT_TABLE_ID/$CURR_DATE
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter LOG_FILE_DIR of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
export LOG_FILE;LOG_FILE=$LOG_FILE_DIR/$TRGT_TABLE_ID.$SRC_JOB_TYPE_ID.normalize.$CURR_DATETIME.log
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter LOG_FILE of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
export RECORD_COUNT_FILE;RECORD_COUNT_FILE=$(
if [[ ! -d $TRGT_IN_DIR ]]
then
   mkdir -p $TRGT_IN_DIR
fi
...
if [[ -n $UOW_TO ]]
then
   RECORD_COUNT_FILE_TMP=$TRGT_IN_DIR/$TRGT_TABLE_ID.record_count.dat
else
   RECORD_COUNT_FILE_TMP=$TRGT_IN_DIR/$TRGT_TABLE_ID.record_count.dat.$SRC_BATCH_SEQ_NUM
fi

print $RECORD_COUNT_FILE_TMP
)
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter RECORD_COUNT_FILE of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
export WATCH_FILE;WATCH_FILE=$(

WATCH_FILE=$TRGT_ETL_ID.$SRC_JOB_TYPE.$SRC_BATCH_SEQ_NUM.done

OLD_DW_SA_WATCHFILEDIR=$DW_WATCH/$SRC_JOB_ENV
...
then
        ...
        DW_SA_WATCHFILEDIR=$DW_WATCH/$SRC_JOB_ENV/$UOW_DATE
        DW_SA_WATCHFILE=$DW_SA_WATCHFILEDIR/$UOW_WATCH_FILE.$UOW_TO
   print $OLD_DW_SA_WATCHFILE $DW_SA_WATCHFILE
else
   print $OLD_DW_SA_WATCHFILE
fi
)
mpjret=$?
if [ 0 -ne $mpjret ] ; then
   print -- Error evaluating: 'parameter WATCH_FILE of single_field_normalize', interpretation 'shell'
   exit $mpjret
fi
(
...
mv "${_AB_PROXY_DIR}" "${AB_JOB}"'-single_field_normalize-ProxyDir'
_AB_PROXY_DIR="${AB_JOB}"'-single_field_normalize-ProxyDir'
print -r -- 'record string("|") node, timestamp, component, subcomponent, event_type; string("|\n") event_text; end' > "${_AB_PROXY_DIR}"'/Normalize_Normalize_on_Vectored_Sub_Records-7.dml'
print -r -- 'void(1)' > "${_AB_PROXY_DIR}"'/Deflate-8.dml'
print -r -- 'record
  string("\n") line;
end;' > "${_AB_PROXY_DIR}"'/Output_log_file-9.dml'
print -r -- 'record
  decimal(" records") rec_cnt = 0;
  string("\n") junk = NULL;
end;' > "${_AB_PROXY_DIR}"'/Redefine_Format_1-10.dml'
print -r -- 'out::rollup(in) =
begin
  out.rec_cnt :: sum(in.rec_cnt);
end;' > "${_AB_PROXY_DIR}"'/Rollup-11.xfr'
print -r -- 'record
  decimal("\n") rec_cnt = 0;
end;' > "${_AB_PROXY_DIR}"'/Rollup-12.dml'
...
mp metadata metadata2 -file "$REFORMAT_DML_FILE"
mp metadata metadata3 -file "$REDEFINE_FORMAT_DML_FILE"
mp metadata metadata4 -file "$OUTPUT_DML"
mp metadata metadata5 -file "${_AB_PROXY_DIR}"'/Normalize_Normalize_on_Vectored_Sub_Records-7.dml'
mp metadata metadata6 -file "${_AB_PROXY_DIR}"'/Deflate-8.dml'
mp metadata metadata7 -file "${_AB_PROXY_DIR}"'/Output_log_file-9.dml'
mp metadata metadata8 -file "${_AB_PROXY_DIR}"'/Redefine_Format_1-10.dml'
mp metadata metadata9 -file "${_AB_PROXY_DIR}"'/Rollup-12.dml'

export AB_CATALOG;AB_CATALOG=${AB_CATALOG:-"${XX_CATALOG}"}
# Catalog Usage: Creating temporary catalog using lookup files only
m_rmcatalog -catalog GDE-single_field_normalize-${AB_JOB}.cat > /dev/null 2>&1
m_mkcatalog -catalog GDE-single_field_normalize-${AB_JOB}.cat
SAVED_CATALOG="${AB_CATALOG}"
export AB_CATALOG;AB_CATALOG='GDE-single_field_normalize-'"${AB_JOB}"'.cat'
# 
# Initialize condition variables to user-specified conditions
# 
AB_USERCOND_single_field_normalize=1
AB_IS_LIVE_single_field_normalize=1
AB_USERCOND_Input_File=1
AB_IS_LIVE_Input_File=1
AB_HAS_DATA_Flow_1=1
AB_USERCOND_Reformat_Determine_Number_of_Normalized_Values=1
AB_IS_LIVE_Reformat_Determine_Number_of_Normalized_Values=1
AB_HAS_DATA_Flow_3=1
AB_USERCOND_Redefine_Format_Create_Vectored_Sub_Records=1
AB_IS_LIVE_Redefine_Format_Create_Vectored_Sub_Records=1
AB_HAS_DATA_Flow_4=1
AB_USERCOND_Normalize_Normalize_on_Vectored_Sub_Records=1
AB_IS_LIVE_Normalize_Normalize_on_Vectored_Sub_Records=1
...
AB_USERCOND_Normalized_Output_File=1
AB_IS_LIVE_Normalized_Output_File=1
AB_USERCOND_Gather_Logs=1
AB_IS_LIVE_Gather_Logs=1
...
if [ X"${AB_VERBOSE_CONDITIONS}" != X"" ]; then
   ...
   # 
   print -r -- 'AB_USERCOND_single_field_normalize=1'
   print -r -- 'AB_IS_LIVE_single_field_normalize=1'
   print -r -- 'AB_USERCOND_Input_File=1'
   print -r -- 'AB_IS_LIVE_Input_File=1'
   print -r -- 'AB_HAS_DATA_Flow_1=1'
   print -r -- 'AB_USERCOND_Reformat_Determine_Number_of_Normalized_Values=1'
   print -r -- 'AB_IS_LIVE_Reformat_Determine_Number_of_Normalized_Values=1'
   print -r -- 'AB_HAS_DATA_Flow_3=1'
   print -r -- 'AB_USERCOND_Redefine_Format_Create_Vectored_Sub_Records=1'
   print -r -- 'AB_IS_LIVE_Redefine_Format_Create_Vectored_Sub_Records=1'
   print -r -- 'AB_HAS_DATA_Flow_4=1'
   print -r -- 'AB_USERCOND_Normalize_Normalize_on_Vectored_Sub_Records=1'
   print -r -- 'AB_IS_LIVE_Normalize_Normalize_on_Vectored_Sub_Records=1'
   print -r -- 'AB_HAS_DATA_Flow_2=1'
   print -r -- 'AB_HAS_DATA_Flow_5=1'
   print -r -- 'AB_USERCOND_Deflate='"${AB_USERCOND_Deflate}"
   print -r -- 'AB_IS_LIVE_Deflate='"${AB_IS_LIVE_Deflate}"
   print -r -- 'AB_HAS_DATA_Flow_8=1'
   print -r -- 'AB_USERCOND_Normalized_Output_File=1'
   print -r -- 'AB_IS_LIVE_Normalized_Output_File=1'
   print -r -- 'AB_USERCOND_Gather_Logs=1'
   print -r -- 'AB_IS_LIVE_Gather_Logs=1'
...
mp ofile Normalized_Output_File $OUTPUT_FILE
mp ifile Output_log_file 'file:'"$LOG_FILE"
AB_PORT_Output_log_file_read=Output_log_file.read
AB_METADATA_Output_log_file_read=' -metadata metadata7'
mp ofile Record_Count_File 'file:'"$RECORD_COUNT_FILE"

# Components in phase 0:
mp reformat-transform Reformat_Determine_Number_of_Normalized_Values -limit 0 -ramp 0.0 -layout Normalized_Output_File
mp add-port Reformat_Determine_Number_of_Normalized_Values.out.out0 ${REFORMAT_XFR_FILE:+"$REFORMAT_XFR_FILE"}
AB_PORT_Reformat_Determine_Number_of_Normalized_Values_out_out0=Reformat_Determine_Number_of_Normalized_Values.out.out0
AB_METADATA_Reformat_Determine_Number_of_Normalized_Values_out_out0=' -metadata metadata2'
mp copy Redefine_Format_Create_Vectored_Sub_Records -layout Normalized_Output_File
AB_PORT_Redefine_Format_Create_Vectored_Sub_Records_out=Redefine_Format_Create_Vectored_Sub_Records.out
AB_METADATA_Redefine_Format_Create_Vectored_Sub_Records_out=' -metadata metadata3'
mp normalize Normalize_Normalize_on_Vectored_Sub_Records "$NORMALIZE_XFR_FILE" -limit 0 -ramp 0.0 -layout Normalized_Output_File
AB_PORT_Normalize_Normalize_on_Vectored_Sub_Records_out=Normalize_Normalize_on_Vectored_Sub_Records.out
AB_METADATA_Normalize_Normalize_on_Vectored_Sub_Records_out=' -metadata metadata4'
AB_PORT_Normalize_Normalize_on_Vectored_Sub_Records_log=Normalize_Normalize_on_Vectored_Sub_Records.log
AB_METADATA_Normalize_Normalize_on_Vectored_Sub_Records_log=' -metadata metadata5'
if [ X"${AB_IS_LIVE_Deflate}" != X0 ]; then
   mp broadcast Deflate -compression "$Deflate__compression" -layout Normalized_Output_File
   AB_PORT_Deflate_out=Deflate.out
   AB_METADATA_Deflate_out=' -metadata metadata6'
else
   AB_PORT_Deflate_out="${AB_PORT_Normalize_Normalize_on_Vectored_Sub_Records_out}"
   AB_METADATA_Deflate_out="${AB_METADATA_Normalize_Normalize_on_Vectored_Sub_Records_out}"
   :
fi
mp logger Gather_Logs 'file:'"$LOG_FILE" Start End -layout layout1
mp checkpoint 0
...
AB_METADATA_Filter_record_count_rows_out=' -metadata metadata7'
mp copy Redefine_Format_1 -layout Record_Count_File
AB_PORT_Redefine_Format_1_out=Redefine_Format_1.out
AB_METADATA_Redefine_Format_1_out=' -metadata metadata8'
mp rollup Rollup '{}' "${_AB_PROXY_DIR}"'/Rollup-11.xfr' -limit 0 -ramp 0.0 -check-sort -layout Record_Count_File
AB_PORT_Rollup_out=Rollup.out
AB_METADATA_Rollup_out=' -metadata metadata9'
mp checkpoint 1
...
mp straight-flow Flow_1 "${AB_PORT_Input_File_read}" Reformat_Determine_Number_of_Normalized_Values.in${AB_METADATA_Input_File_read} -compressed
mp straight-flow Flow_3 "${AB_PORT_Reformat_Determine_Number_of_Normalized_Values_out_out0}" Redefine_Format_Create_Vectored_Sub_Records.in${AB_METADATA_Reformat_Determine_Number_of_Normalized_Values_out_out0}
mp straight-flow Flow_4 "${AB_PORT_Redefine_Format_Create_Vectored_Sub_Records_out}" Normalize_Normalize_on_Vectored_Sub_Records.in${AB_METADATA_Redefine_Format_Create_Vectored_Sub_Records_out}
let AB_FLOW_CONDITION="AB_IS_LIVE_Deflate"
if [ X"${AB_FLOW_CONDITION}" != X0 ]; then
   mp straight-flow Flow_2 "${AB_PORT_Normalize_Normalize_on_Vectored_Sub_Records_out}" Deflate.in${AB_METADATA_Normalize_Normalize_on_Vectored_Sub_Records_out}
fi
mp straight-flow Flow_8 "${AB_PORT_Deflate_out}" Normalized_Output_File.write${AB_METADATA_Deflate_out}
mp fan-in-flow Flow_5 "${AB_PORT_Normalize_Normalize_on_Vectored_Sub_Records_log}" Gather_Logs.in${AB_METADATA_Normalize_Normalize_on_Vectored_Sub_Records_log}
...
unset AB_COMM_WAIT
export AB_TRACKING_GRAPH_THUMBPRINT;AB_TRACKING_GRAPH_THUMBPRINT=1012494
mp run
mpjret=$?
unset AB_COMM_WAIT
