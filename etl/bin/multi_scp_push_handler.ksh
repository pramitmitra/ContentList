#!/bin/ksh -eu
# Title:        Multi SCP Push Handler
# File Name:    multi_scp_push_handler.ksh
# Description:  Handle multiple scp push
# Developer:    ???
# Created on:
# Location:     $DW_EXE
# Logic:
...
#---------    -----  ---------------------------  ------------------------------
# ????-??-??   1.0    ???                          Initital
# 2012-11-26   2.0    Ryan Wong                    Add UOW_[FROM/TO]_TIME, SCP_UOW_[FROM/TO]_DATE_RFMT_CODE
# 2013-03-25   2.1    Jacky Shen                   Add a check to Infra_Run complete file to fail job if the complete file still exists
# 2013-04-23   2.2    Ryan Wong                    Update OUT_DIR for UOW directory path to include UOW HH/MM/SS
...
export JOB_TYPE=scp_push
export JOB_TYPE_ID=scp_push
...
then
   ...
   assignTagValue SCP_UOW_FROM_DATE_RFMT_CODE SCP_UOW_FROM_DATE_RFMT_CODE $ETL_CFG_FILE W 0
   assignTagValue SCP_UOW_TO_DATE_RFMT_CODE SCP_UOW_TO_DATE_RFMT_CODE $ETL_CFG_FILE W 0
   export UOW_FROM_DATE_RFMT=$($DW_MASTER_EXE/dw_infra.reformat_date.ksh $UOW_FROM_DATE $SCP_UOW_FROM_DATE_RFMT_CODE)
   export UOW_TO_DATE_RFMT=$($DW_MASTER_EXE/dw_infra.reformat_date.ksh $UOW_TO_DATE $SCP_UOW_TO_DATE_RFMT_CODE)
...
export PARENT_ERROR_FILE=$DW_SA_LOG/$TABLE_ID.$JOB_TYPE_ID.multi_scp_push_handler${UOW_APPEND}.$CURR_DATETIME.err
export PARENT_LOG_FILE=$DW_SA_LOG/$TABLE_ID.$JOB_TYPE_ID.multi_scp_push_run${UOW_APPEND}.$CURR_DATETIME.log
...
then
   ...
   set +e
   (time $DW_MASTER_BIN/dw_infra.multi_scp_push_run.ksh) > $PARENT_LOG_FILE 2>&1
   RUN_RCODE=$?
