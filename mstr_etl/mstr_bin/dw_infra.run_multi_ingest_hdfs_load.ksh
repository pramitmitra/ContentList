#!/bin/ksh -eu
#------------------------------------------------------------------------------------------------
# Filename:     dw_infra.run_multi_ingest_hdfs_load.ksh
#
# Revision History:
#
...
do
  ...
  then
    ...
    LOG_FILE=$DW_SA_LOG/$TABLE_ID.$JOB_TYPE_ID.$RECORD_ID.single_hadoop_load_file.$CURR_DATETIME.log

    print "Processing RECORD_ID: $RECORD_ID, SOURCE_FILE: $SOURCE_FILE, HADOOP CONN: $HDP_CONN  `date`"

    COMMAND="$DW_MASTER_BIN/dw_infa.single_ingest_hdfs_load.ksh $ETL_ID $RECORD_ID $HDP_CONN > $LOG_FILE 2>&1"
