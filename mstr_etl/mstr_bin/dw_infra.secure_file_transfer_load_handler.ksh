#!/bin/ksh -eu
# Title:        Secure File Transfer Load Handler
# File Name:    dw_infra.secure_file_transfer_load_handler.ksh
...
# read in UC4 runtime parameters, they will be used in teradata query band if it is an load job from teradata
...
export PARENT_LOG_FILE=$DW_SA_LOG/$TABLE_ID.$JOB_TYPE_ID.secure_file_transfer_load_run${UOW_APPEND}.$CURR_DATETIME.log
...
then
   ...
   set +e
   (time $DW_MASTER_BIN/dw_infra.secure_file_transfer_load_run.ksh) > $PARENT_LOG_FILE 2>&1
   RUN_RCODE=$?
