...
# 2016-04-21     1.4   Michael Weng                 Check on hd* instead of specific hd1/hd2/hd3
###################################################################################################################
...
then
   ...
   LOG_FILE=$DW_SA_LOG/$TABLE_ID.$JOB_TYPE_ID.target_table_load.${SQL_FILE_BASENAME}${UOW_APPEND}.$CURR_DATETIME.log
        ...
        then
           ...
           set +e
           $DW_MASTER_BIN/dw_infra.runTDSQL_sa.ksh $ETL_ID $JOB_ENV $SQL_FILE $UOW_PARAM_LIST > $LOG_FILE 2>&1
           rcode=$?
           set -e
        fi
