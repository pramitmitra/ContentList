#!/bin/ksh -eu
#------------------------------------------------------------------------------------------------
# Filename:     dw_infra.multi_sft_push_run.ksh
#
# Revision History:
#
...
# get LAST_EXTRACT_VALUE from last_sft_value.dat
#if [ -f $DW_SA_DAT/$TABLE_ID.*.last_sft_value.dat ]; then
#export FROM_EXTRACT_VALUE=$(<`ls $DW_SA_DAT/$TABLE_ID.*.last_sft_value.dat | cut -d' ' -f1 | head -1`)
#fi
...
##########################################################################################################
#
# Beginning SFT push for ETL_ID: $ETL_ID, BATCH_SEQ_NUM: $BATCH_SEQ_NUM   `date`
#
##########################################################################################################
"
...
then
	...
	print "Running sft_cleanup.ksh for JOB_ENV: $JOB_ENV, JOB_TYPE_ID: $JOB_TYPE_ID  `date`"
...
######################################################################################################
# 
#	Setting up SFT push process specific variables for database and file extraction
#
######################################################################################################

set +e
grep "^SFT_PUSH_RUN_ENV\>" $DW_CFG/$ETL_ID.cfg | read PARAM SFT_PUSH_RUN_ENV COMMENT
ecode=$?
set -e

if [ $rcode != 0 ]
then
        print "${0##*/}:  ERROR, failure determining value for SFT_PUSH_RUN_ENV parameter from $DW_CFG/$ETL_ID.cfg" >&2
        exit 4
fi

if [ $SFT_PUSH_RUN_ENV != $JOB_ENV ]
then
	print "${0##*/}: The SFT push will not take place in this job environment run."
	tcode=0
	exit
fi

set +e
grep "^SFT_PUSH_SOURCE_ENV\>" $DW_CFG/$ETL_ID.cfg | read PARAM SFT_PUSH_SOURCE_ENV COMMENT
rcode=$?
set -e

if [ $rcode != 0 ]
then
        print "${0##*/}:  ERROR, failure determining value for SFT_PUSH_SOURCE_ENV parameter from $DW_CFG/$ETL_ID.cfg" >&2
        exit 4
fi

export DATA_PROCESS_MSG=single_sft_push
export DATA_CONN_TYPE=sft

set +e
grep "^CNDTL_SFT_PUSH_TO_EXTRACT_VALUE\>" $DW_CFG/$ETL_ID.cfg | read PARAM CNDTL_SFT_PUSH_TO_EXTRACT_VALUE COMMENT
rcode=$?
set -e

if [ $rcode != 0 ]
then
	print "${0##*/}:  ERROR, failure determining value for CNDTL_SFT_PUSH_TO_EXTRACT_VALUE parameter from $DW_CFG/$ETL_ID.cfg" >&2
	exit 4
fi

export CNDTL_SFT_PUSH_TO_EXTRACT_VALUE
...
set +e
grep -s "^$DATA_PROCESS_MSG\>" $COMP_FILE >/dev/null
RCODE=$?
set -e
...
then
	...
	if [[ $TABLE_COUNT -eq 1 ]]
	then
		print "Processing single sft push for TABLE_ID: $TABLE_ID  `date`"
    # get LAST_EXTRACT_VALUE from last_sft_value.dat
    if [ -f $DW_SA_DAT/$TABLE_ID.*.last_sft_value.dat ]; then
    export FROM_EXTRACT_VALUE=$(<`ls $DW_SA_DAT/$TABLE_ID.*.last_sft_value.dat | cut -d' ' -f1 | head -1`)
    fi
    ...
    then
		...
		LOG_FILE=$DW_SA_LOG/$TABLE_ID.$JOB_TYPE_ID.single_sft_push.$CURR_DATETIME.log

		set +e
		eval $DW_MASTER_BIN/dw_infra.single_sft_push.ksh -e $ETL_ID -i $FILE_ID -c $DBC_FILE -s $TABLE_NAME -t $DATA_FILENAME > $LOG_FILE 2>&1
		rcode=$?
		set -e
	...
	then
		print "Processing multiple sft extracts for SCP_FILE_ID: $TABLE_ID  `date`"

		export MULTI_COMP_FILE=$DW_SA_TMP/$TABLE_ID.multi_sft_push.complete
		export PARENT_LOG_FILE=$DW_SA_LOG/$TABLE_ID.$JOB_TYPE_ID.run_sft_push.$CURR_DATETIME.log
		export ERROR_FILE=$DW_SA_LOG/$TABLE_ID.$JOB_TYPE_ID.single_sft_push.$CURR_DATETIME.err  # job error file
		...
		# remove previous $DATA_CONN_TYPE list files to ensure looking for the correct set of data files for this run.
		rm -f $DW_SA_TMP/$TABLE_ID.*.$DATA_CONN_TYPE.*.lis

		# Create a list of files to be processed per extract database server.
    while read FILE_ID_T balabala
    do
    	if [ -f $DW_SA_DAT/$TABLE_ID.$FILE_ID_T.last_sft_value.dat ]; then
         export FROM_EXTRACT_VALUE=$(<$DW_SA_DAT/$TABLE_ID.$FILE_ID_T.last_sft_value.dat)
      fi
      set +e
	    grep "^SFT_PUSH_TO_EXTRACT_VALUE_FUNCTION\>" $DW_CFG/$ETL_ID.cfg | read PARAM SFT_PUSH_TO_EXTRACT_VALUE_FUNCTION COMMENT
	    rcode=$?
		...
		for FILE in $(ls $DW_SA_TMP/$TABLE_ID.*.$DATA_CONN_TYPE.*.lis)
		do
			DBC_FILE=${FILE#$DW_SA_TMP/$TABLE_ID.}
			DBC_FILE=${DBC_FILE%%.*}

			LOG_FILE=$DW_SA_LOG/$TABLE_ID.$JOB_TYPE_ID.$DBC_FILE.run_multi_sft_push.$CURR_DATETIME.log
			print "Running run_multi_sft_push.ksh $FILE  `date`"
			COMMAND="$DW_MASTER_BIN/dw_infra.run_multi_sft_push.ksh -f $FILE -c $DBC_FILE -l $LOG_FILE > $LOG_FILE 2>&1"
	...
	print "$DATA_PROCESS_MSG" >> $COMP_FILE

elif [ $RCODE = 0 ]
then
	print "$DATA_PROCESS_MSG process already complete"
else
	exit $RCODE
fi
...
##########################################################################################################
#
# SFT push for ETL_ID: $ETL_ID, BATCH_SEQ_NUM: $BATCH_SEQ_NUM complete   `date`
#
##########################################################################################################"
